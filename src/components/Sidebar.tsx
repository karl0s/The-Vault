...
export function Sidebar({ artists, activeLetter, onLetterClick, onArtistClick }: SidebarProps) {
  ...
  return (
    <>
      <aside className="fixed left-0 top-16 bottom-0 w-16 bg-black/50 backdrop-blur-sm border-r border-white/10 flex flex-col items-center py-6 z-30 overflow-y-auto">
        <button 
      ...
      {hoveredLetter && artistsByLetter[hoveredLetter] && (
        <>
          ...
          <div 
            className="fixed left-16 w-64 bg-[#181818] border border-white/10 z-50 overflow-y-auto shadow-2xl rounded-r-lg"
            style={{ top: `${menuPosition.top}px`, maxHeight: '400px' }}
            onMouseEnter={handleMenuMouseEnter}
            onMouseLeave={handleMenuMouseLeave}
          >
            <div className="p-3">
