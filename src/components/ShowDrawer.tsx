...
import { X, Clock, HardDrive, Film, Music, Info } from 'lucide-react';
...
export function ShowDrawer({ show, onClose, onImageClick, getImageUrl }: ShowDrawerProps) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className="fixed top-0 right-0 bottom-0 w-full md:w-[600px] lg:w-[700px] bg-[#181818] z-50 overflow-y-auto animate-in slide-in-from-right duration-300">
        <div className="relative">
          ...
          <div className="relative h-80 bg-gradient-to-b from-black/50 to-[#181818]">
            {images.length > 0 ? (
              <>
                <ImageWithFallback
                  src={images[0]}
          ...
          <div className="p-8 space-y-8">
            ...
            {(locationStr || show.VenueName || show.EventOrFestival) && (
              <div className="space-y-1">
                {locationStr && <p className="text-gray-300">{locationStr}</p>}
                {show.VenueName && <p className="text-gray-400 text-sm">{show.VenueName}</p>}
            ...
            {/* Recording Info */}
            {(show.RecordingType || show.Generation || show.SourceEquipment) && (
              <div>
                ...
                <div className="bg-black/30 rounded-lg p-4 space-y-2 text-sm">
                  ...
                  {show.Generation && (
                    <div>
                      ...
                      <span className="text-white">{show.Generation}</span>
                    </div>
                  )}
                  {show.SourceEquipment && (
                    <div>
                      <span className="text-gray-400">Equipment:</span>{' '}
                      <span className="text-white">{show.SourceEquipment}</span>
                    </div>
                  )}
                  {show.MasterDriveName && (
                    <div>
                      <span className="text-gray-400">Drive:</span>{' '}
                      <span className="text-white">{show.MasterDriveName}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {show.Notes && (
              <div>
                <h3 className="text-sm text-gray-400 mb-3">Notes</h3>
            ...
            {/* Source / Lineage */}
            {(show.Lineage || show.FolderPath || videoFiles.length > 0) && (
              <div>
                <h3 className="text-sm text-gray-400 mb-3">Source & Files</h3>
